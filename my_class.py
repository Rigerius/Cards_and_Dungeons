...
class Button:
    """Базовый класс кнопки для использования в Card"""
    ...
    def draw(self):
        ...
        rect = arcade.rect.XYWH(
            self.x - self.width / 2,  # левая координата
            self.y - self.height / 2,  # нижняя координата
            self.width,
            self.height
        )
        arcade.draw_rect_filled(rect, current_color)

        # Контур прямоугольника (используем тот же прямоугольник)
    ...
    def check_mouse_hover(self, x, y):
        """Проверяет, наведен ли курсор на кнопку"""
        self.is_hovered = (
                self.is_visible and
...
class Card(Button):
    ...
    def __init__(self, x, y, width, height, text,
        ...
        super().__init__(x, y, width, height, text,
                         card_color, hover_color, text_color, font_size)
        ...
        # Обновляем текстовый объект с правильной позицией
...
class F_Player:
    ...
    def __init__(self, name, image_path, max_hp=100, max_mana=10):
        self.name = name
        self.max_hp = max_hp
        self.current_hp = max_hp
        ...
        self.image_y = SCREEN_HEIGHT // 2 + 30  # Выше центра
        ...
        self.heart_y = SCREEN_HEIGHT - 80  # Вверху экрана
